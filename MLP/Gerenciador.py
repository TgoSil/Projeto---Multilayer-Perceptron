...
class Gerenciador:
    ...
    def avaliaRecall(self, matriz_confusao):
        ...
        for i, linha in enumerate(matriz_confusao):
            ...
            # Se o total da classe for maior que zero, calcula normal. Se for zero, define como 0.0
            total_classe = linha[i] # ou np.sum(matriz_confusao[i, :])
            if total_classe > 0:
                resultado_recall[i] = linha[i] / total_classe
            else:
                resultado_recall[i] = 0.0
